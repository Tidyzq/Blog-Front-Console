...
export const language: languages.IMonarchLanguage = {
  ...
  tokenizer: {
    root: [
      ...
      // latex math block (2 dollar)
      [ /^\s*\$\$\s*$/, { token: TOKEN_MATH_BLOCK, next: '@mathblock' }],
      // markup within lines
      { include: '@linecontent' },
    ],
    codeblock: [
    ...
    // latex math block
    mathblock: [
      [ /^\s*\$\$\s*$/, { token: TOKEN_MATH_BLOCK, next: '@pop' }],
      [ /.*$/, TOKEN_BLOCK_CODE ],
    ],
    linecontent: [
      // escapes
